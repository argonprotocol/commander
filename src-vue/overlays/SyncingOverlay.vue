    ...
    <div class="absolute inset-0 z-100 overflow-y-auto pt-[1px] flex items-center justify-center pointer-events-none">
      <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
        <div class="flex flex-col w-6/12 bg-white border border-black/40 px-7 pb-8 rounded-lg pointer-events-auto shadow-xl relative overflow-scroll">
          ...
          <div v-if="stats.syncError" class="flex flex-row items-center gap-x-2">
            ...
            <button class="text-red-500 border border-red-500 rounded-md px-3 text-md cursor-pointer hover:bg-red-500 hover:text-white" @click="retrySync">
...
async function retrySync() {
  ...
  await invoke('retry_stats_sync', {});
  stats.value.syncError = null;
  isRetrying.value = false;
  updateStats();
}

async function updateStats() {
  const cohortId = stats.value.dashboard.cohort_id || null;
  stats.value = await invoke('fetch_stats', { cohortId });
  if (stats.value.syncError) {
    console.log('SYNCING ERROR: ', stats.value.syncError);
  } else if (stats.value.syncProgress < 100) {
