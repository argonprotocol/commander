<template>
  <div>
    <div class="flex flex-row justify-between items-center w-full pt-2 pb-3 space-x-1 border-b border-black/20">
      ...
      <div @click="closeOverlay" class="mr-2 z-10 flex items-center justify-center text-sm/6 font-semibold cursor-pointer border rounded-md w-[30px] h-[30px] focus:outline-none border-slate-400/60 hover:border-slate-500/60 hover:bg-[#f1f3f7]">
      ...
      <div class="flex flex-col grow pt-2 text-md">
        <div v-if="config.biddingRules">
          ...
          <p class="mt-2 font-light">
            Based on the rules you configured, your Bidding Bot needs the following tokens in order to optimize its ability to win seats:
          </p>
          ...
          <p class="mt-6">The above table shows the optimal amounts desired. However, your Bot cannot operate if it has anything less than the following:</p>
        ...
        <div v-else>
          You haven't set any bidding rules. Please do so. 
        </div>
      </div>
