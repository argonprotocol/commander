  ...
  <div class="flex flex-row items-end gap-2 w-full">
    <div class="flex flex-col w-1/2">
      ...
      <div class="flex flex-row items-center gap-2 w-full">
        <InputNumber v-model="config.vaultingRules.capitalForSecuritizationPct" @update:modelValue="handleSecurityChange" :min="50" :max="100" :dragBy="1" :dragByMin="0.1" :maxDecimals="1" format="percent" class="w-full" />
      </div>
    </div>
...
function handlePoolChange(value: number) {
  config.vaultingRules.capitalForSecuritizationPct = 100 - value;
}
