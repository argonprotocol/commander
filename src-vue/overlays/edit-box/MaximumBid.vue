...
<template>
  ...
  <div v-if="options.length" class="flex flex-row items-center justify-between">
    <InputMenu v-model="config.biddingRules.finalBidAmountFormulaType" :options="options" />
  </div>

  <template v-if="config.biddingRules.finalBidAmountFormulaType === BidAmountFormulaType.Custom">
    <label class="mt-3 font-bold opacity-60 mb-0.5">
      Value
    </label>
    <InputArgon v-model="config.biddingRules.finalBidAmountCustom" :min="0n" class="min-w-60" />
  </template>
  <template v-else>
    <label class="mt-3 font-bold opacity-60 mb-0.5">
    ...
    <div v-if="options.length" class="flex flex-row items-center justify-between space-x-3">
      <InputMenu v-model="config.biddingRules.finalBidAmountAdjustmentType" :options="[
      ...
      <InputArgon v-if="isAbsoluteType" v-model="config.biddingRules.finalBidAmountAbsolute" class="min-w-60 w-1/2" />
      <InputNumber v-else v-model="config.biddingRules.finalBidAmountRelative" :min="-100" :dragBy="0.01" format="percent" class="w-1/2" />
    </div>
  </template>
</template>
...
const isAbsoluteType = Vue.computed(
  () => config.biddingRules.finalBidAmountAdjustmentType === BidAmountAdjustmentType.Absolute,
);
