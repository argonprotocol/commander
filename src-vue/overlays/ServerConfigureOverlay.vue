    ...
    <div class="absolute inset-0 z-100 overflow-y-auto pt-[1px] flex items-center justify-center pointer-events-none">
      <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
        <div class="bg-white border border-black/40 px-7 pt-6 pb-4 rounded-lg pointer-events-auto shadow-xl relative w-160 min-h-60 overflow-scroll">
          ...
          <p>
            Changing the IP address of your cloud machine will not uninstall or stop any mining software that is already running. It will only switch Commander to use the new server.
          </p>

          <label class="text-sm font-bold text-slate-800/70 mt-6 mb-2 block">Public SSH Key (ensure this is added to your server)</label>
          <CopyToClipboard ref="copyToClipboard" @click="highlightCopiedContent" :content="config.serverDetails.sshPublicKey" class="relative mb-3">
            <input type="text" :value="config.serverDetails.sshPublicKey" class="bg-white py-3 pl-3 pr-8 border border-slate-300 rounded-md w-full pointer-events-none" readonly />
            ...
            <template #copied>
              <div class="bg-white py-3 pl-3 pr-8 border border-slate-300 rounded-md w-full pointer-events-none overflow-hidden">
                <span class="bg-blue-200 whitespace-nowrap w-full inline-block">{{ config.serverDetails.sshPublicKey }}</span>
              </div>
              <div class="flex flex-row items-center absolute right-[1px] top-1/2 -translate-y-1/2 pointer-events-none bg-white pl-2 pr-[15px]">
          ...
          <div v-else-if="hasServerDetailsError" class="grow rounded-md bg-red-200 p-2 pl-4 flex items-center">
            <div class="flex">
              ...
              <div class="ml-3">
                <h3 class="text-sm font-medium text-red-800">Failed to connect to server. Please ensure you used the correct Public Key.</h3>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-row justify-end gap-4 mt-10 border-t border-slate-300 pt-4">
            ...
            <button @click="updateServer" class="bg-argon-button border border-argon-button-hover hover:bg-argon-button-hover text-white font-bold inner-button-shadow px-6 py-2 rounded-md cursor-pointer">
