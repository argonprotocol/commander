...
import { IConfigInstallDetails, IConfigInstallStep, InstallStepErrorType, InstallStepStatus, InstallStepKey } from '../interfaces/IConfig';
...
export class InstallerCheck {
  ...
  public get isServerInstallComplete(): boolean {
    return this.config.installDetails.UbuntuCheck.progress >= 100
  ...
  private extractFilenameStatus(stepName: InstallStepKey, stepOldData: IConfigInstallStep, filenames: String[]): 'Pending' | 'Started' |'Finished' | 'Failed' {
  ...
  private async calculateFinalStepProgress(
    stepPending: IConfigInstallStep, 
  ): Promise<number> {
