<template>
  <Popover class="relative pointer-events-auto" v-slot="{ open: isOpen }">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
      <PopoverPanel v-slot="{ close }" ref="popoverElem" class="absolute -left-[1px] z-100 mt-2 flex w-screen max-w-min">
        ...
        <div class="flex flex-col p-1 shrink rounded bg-white text-sm/6 font-semibold text-gray-900 shadow-lg ring-1 ring-gray-900/20">
          <a v-for="(record, id) of currencyStore.displayCurrencies as Record<ICurrency, ICurrencyRecord>" :key="id" @click="() => { setDisplayCurrency(id); close(); }" :class="currencyStore.displayCurrency.id === id ? '!text-indigo-500' : '!text-gray-900'" class="group/item flex flex-row justify-between py-1 px-2 border-b last:border-b-0 border-argon-menu-hover hover:!text-indigo-600 hover:bg-argon-menu-hover cursor-pointer">
            <span  :class="currencyStore.displayCurrency.id === id ? 'opacity-100' : 'opacity-40'" class="font-medium group-hover/item:opacity-100 mr-4">{{record.name}}</span>
