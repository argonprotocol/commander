<template>
  <div
    ...
    <div
      ...
      :class="[i > maxFilled20 ? 'bg-slate-300/20' : colorFill]"></div>
    <div
      v-for="i in 10"
      :key="i"
      class="block h-full w-1/20 min-w-1 rounded-full xl:hidden"
      :class="[i > maxFilled20 ? 'bg-slate-300/20' : colorFill]"></div>
    <div class="text-slate-900/50">{{ numeral(percent).format('0') }}%</div>
  </div>
</template>
