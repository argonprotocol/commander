...
export class Mainchain {
  ...
  public async getOwnershipAmountMinimum(): Promise<number> {
    ...
    const value = await client.query.miningSlot
  ...
  public async currentMinimumRewardsAtTick(
    currentTick: bigint,
  ): Promise<number> {
    ...
    const currentReward =
      initialReward + numIntervals * BLOCK_REWARD_INCREASE_PER_INTERVAL;
  ...
  public async argonBlockRewardsForFullYear(
    currentRewardsPerBlock: number,
  ): Promise<number> {
    const intervalsPerYear = (365 * 1440) / BLOCK_REWARD_INTERVAL;
    const currentTick = await this.getCurrentTick();
    const startingRewardsPerBlock =
      await this.currentMinimumRewardsAtTick(currentTick);
    ...
    for (let i = 0; i < intervalsPerYear; i++) {
      ...
      minimumRewardsPerBlock = Math.min(
        minimumRewardsPerBlock,
        BLOCK_REWARD_MAX,
      );
      currentRewardsPerBlock = Math.max(
        minimumRewardsPerBlock,
    ...
    if (intervalsPerYearRemainder > 0) {
      totalRewards +=
        currentRewardsPerBlock *
        (BLOCK_REWARD_INTERVAL * intervalsPerYearRemainder);
    }
  ...
  public async getCurrentArgonTargetPrice(): Promise<number> {
    const client = await this.client;
    const argonPrice = (
      await client.query.priceIndex.current()
    ).unwrapOrDefault();
    return convertFixedU128ToBigNumber(
      argonPrice.argonUsdTargetPrice.toBigInt(),
    ).toNumber();
  }
  ...
  public async getMiningSeatCount(): Promise<number> {
    const client = await this.client;
    const activeMiners = (
      await client.query.miningSlot.activeMinersCount()
    ).toNumber();
  ...
  public async getAggregateBidCosts(): Promise<number> {
    const client = await this.client;
    const activeMiners =
      await client.query.miningSlot.activeMinersByIndex.entries();
  ...
  public async getAggregateBlockRewards(): Promise<{
    ...
    for (const [cohortId, blockReward] of blockRewards) {
      const fullRotationsSinceCohortStart =
        currentCohortId - cohortId.toBigInt();
      const ticksSinceCohortStart =
        fullRotationsSinceCohortStart * 1_440n + ticksElapsedToday;
      // const startingTick = currentTick - ticksSinceCohortStart;
      // const endingTick = currentTick;
      const argonsMinedInCohort =
        (blockReward.toBigInt() * ticksSinceCohortStart) / 10n;
  ...
  public async fetchArgonsInCirculationMinusBitcoinLocked() {
    ...
    const argonsInCirculation = (
      await client.query.balances.totalIssuance()
    ).toNumber();
    const bitcoinArgons = (
      await client.query.mint.mintedBitcoinArgons()
    ).toNumber();
  ...
  public async fetchCurrentRewardsPerBlock() {
    const client = await this.client;
    return (
      (await client.query.blockRewards.argonsPerBlock()).toNumber() /
      MICROGONS_PER_ARGON
    );
  }
  ...
  public async fetchExchangeRates(): Promise<{ USD: number, ARGNOT: number, ARGN: number, BTC: number }> {
    ...
    const USD = convertFixedU128ToBigNumber(
      priceIndex.argonUsdPrice.toBigInt(),
    ).toNumber();
    const ARGNOT =
      convertFixedU128ToBigNumber(
  ...
  public async getNextSlotRange(): Promise<[bigint, bigint]> {
    const client = await this.client;
    const nextSlotRangeBytes = await client.rpc.state.call(
      'MiningSlotApi_next_slot_era',
      '',
    );
    const nextSlotRangeRaw = client.createType(
      '(u64, u64)',
      nextSlotRangeBytes,
    );
